// #include <string.h>  потом убрать, когда всё проверим
...
long double round_to_precision(long double value, int precision) {
  long double factor =
      pow(10.0, precision);  // Определяем множитель для округления
  long double int_part;
  long double frac_part = my_abs(
      modfl(value * factor, &int_part));  // Разделяем целую и дробную части
...
//     long double int_part;
//     long double frac_part = my_abs(modfl(num, &int_part)); // Разделяем целую
//     и дробную части if(my) long long int fpart = 0; long long int pow = 1;
...
int float_number(char *str, specification_read spec_read, long double num) {
  ...
  ftos(str, num, spec_read.accuracy, spec_read.flag_L,
       spec_read.accuracy > 0 || spec_read.flag_hash);
  return s21_strlen(str);
}

long double prepare_science(specification_read spec_read, long double num,
                            int *e) {
  *e = 0;
  if (num == 0.0) {
    *e = 0;
...
int digit_specificator(char *str, specification_read spec_read,
  ...
  if (spec_read.spec == 'f') {
    ...
    int is_e = int_len > spec_read.accuracy || my_abs(num) < 0.0001 ||
               my_abs(num) > 999999;
    int e;
    if (is_e) num = prepare_science(spec_read, num, &e);
...
int s_specificator(char *str, specification_read spec_read, char *str2) {
  ...
  spec_read.lenght = fmax(
      0, spec_read.lenght - (int)str_len);  //(int) bcs double gets to big:/
  if (spec_read.flag_minus) {
...
char *ftos(char *res, long double num, int accuracy, int is_long,
           int need_dot) {
...
//     // int rez =         s21_sprintf(res, format1, 5555555.5555555,
//     555555.555555, 5555.55555, 55.55555555, 555.55555555, 0.55555555,
//     0.055555555, 0.0055555555, 0.00055555555, 0.000055555555,
//     0.00000055555555);
//     // int real_rez1 = sprintf(real_res1, format1, 5555555.5555555,
//     555555.555555, 5555.55555, 55.55555555, 555.55555555, 0.55555555,
//     0.055555555, 0.0055555555, 0.00055555555, 0.000055555555,
//     0.00000055555555);
...
void *s21_insert(const char *src, const char *str, s21_size_t start_index) {
  ...
  char *new_str = S21_NULL;
  if (src_len > 0) new_str = calloc(sum_len, sizeof(char));
